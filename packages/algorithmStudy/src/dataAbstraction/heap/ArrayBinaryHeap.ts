...
export class ArrayBinaryHeap<T extends number>{
    ...
    exchange(...arg: any[]) {
        return exchange.apply(this, arg)
    }
    ...
    maxSwim(index: number) {
        ...
        while (index > 1 && less<T>(this.list[index >> 1], this.list[index], this._compareFunc)) {
            this.exchange(this.list, index >> 1, index)
            index = index >> 1
        }
    }
    ...
    maxSink(index: number) {
        ...
        while (index * 2 <= len) {
            ...
            this.exchange(this.list, firstNum, index)
            index = firstNum
        }
    }
    ...
    delTop() {
        ...
        // 将顶部的值和最后一个值进行交换
        this.exchange(this.list, 1, this.count--)
        // 取出最后一个值
...
export interface IndexArrayBinaryHeapProps<T> {
    type: HeapType;
    compareFunc?: (p: T, q: T) => number;
}


export class IndexArrayBinaryHeap<T extends number>{
    private list: Array<T> = []; // 队列
    private count: number = 0; // 计数
    type: HeapType; // 堆类型
    protected _compareFunc: (p: T, q: T) => number = null; // 比较方法
    ...
    private sinkMap: BaseObject; // 下沉map
    private isHeapMap: BaseObject; // 堆判断map
    private indexList: number[] = []; // 索引列表
    private maxCount: number = 0; // 堆的最大数量
    private countList: number[] = []; // count 的索引 
    constructor(props: IndexArrayBinaryHeapProps<T>) {
        if (props) {
            this.type = props.type
        } else {
            throw new RangeError(`type is must,place set heap type`)
    ...
    */
    ...
    validateIndex(index: number) {
        if (index < 0) {
            throw new RangeError(`index is negative ${index}`)
        }
